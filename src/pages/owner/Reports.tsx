...
export default function Reports() {
  ...
  const filteredTx = useMemo(() => {
    return transactions.filter((t) => {
      ...
      if (
        toDate &&
        ts > new Date(toDate).getTime() + 86400000
      ) {
        return false;
      }
  ...
  const trend = useMemo(() => {
    ...
    for (const t of filteredTx) {
      ...
      if (slot) {
        slot.total += Math.max(0, t.amount);
      }
    }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          ...
          {
            l: "Active tenants",
            v: activeTenants.toLocaleString(),
          },
          {
            l: "Total units",
            v: totalUnits,
      ...
      {canUseAdvancedReports ? (
          ...
          <div className="rounded-xl border border-border bg-gradient-card p-5">
            ...
            {trend.every((m) => m.total === 0) ? (
              <div className="h-48 grid place-items-center text-center text-sm text-muted-foreground">
                ...
                <br />
                Charts populate as transactions are recorded.
              </div>
            ) : (
        ...
        <div className="rounded-xl border border-dashed border-primary/40 bg-primary/5 p-8 text-center space-y-3">
          ...
          <div className="font-display font-semibold">
            Advanced reports are on Growth & Scale
          </div>

          <div className="text-sm text-muted-foreground">
            Your {planLabel} plan shows top-line metrics.
            Upgrade to unlock revenue charts, exports,
            and trend analysis.
          </div>
